...
def train_SN(model, optimizer, scheduler, episodes=1):
    ...
    for episode in range(episodes):
        ...
        # start training
        scores = torch.zeros(NUM_CL,(NUM_EX+NUM_CL)).to(device=device, dtype=dtype)
        targets = torch.zeros(NUM_CL,(NUM_EX+NUM_CL)).to(device=device, dtype=dtype)
        sample_count = 0
...
def TrainTheModel():
    ...
    snet_scheduler = StepLR(snet_optim,step_size=5000,gamma=0.5)
